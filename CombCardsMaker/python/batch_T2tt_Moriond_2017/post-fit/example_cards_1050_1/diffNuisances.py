...
parser.add_option('--print', dest='doprint', default=False, action='store_true', help='taggle print or not')
...
mapper_break = {'qcd_invertDphi': ['invertDphi'], 'qcd_contamUnc':['contamUnc'], 'qcd_other':['nonclosure', 'tfactor'], 'rare':['rare'], 'signal':['signal'], 'ttbarW':['ttbarW'], 'ttz':['ttz'], 'zinv':['zinv']}

mapped_names = {}
for i in range(fpf_s.getSize()):
    nuis_s = fpf_s.at(i)
    name   = nuis_s.GetName();
    for key, val in mapper_break.items():
       for iv in val:
          if iv in name:
             if key in mapped_names.keys():
                mapped_names[key].append(name)
             else:
                mapped_names[key] = [name]

mapped_hists = {}
mapped_counters = {}
for key, val in mapped_names.items():
   if not (key in mapped_hists.keys()):
      mapped_hists[key] = {key+'_prefit_fit_b': ROOT.TH1F(key+'_prefit_fit_b', "B-only fit Nuisances;;#theta ", len(val), 0, len(val))}
      mapped_hists[key].update({key+'_prefit_fit_s': ROOT.TH1F(key+'_prefit_fit_s', "S+B fit Nuisances;;#theta ", len(val), 0, len(val))})
      mapped_hists[key].update({key+'_prefit_nuisances': ROOT.TH1F(key+'_prefit_nuisances', "Prefit Nuisances;;#theta ", len(val), 0, len(val))})
      mapped_hists[key].update({key+'_pull': ROOT.TH1F(key+'_pulls', "Pull;;#Delta x/#sigma", len(val), 0, len(val))})
      mapped_counters[key] = 0
...
for i in range(fpf_s.getSize()):
    ...
    for key, val in mapped_names.items():
       ...
       if not found:
          mapped_key = ''
#    print('mapped_key : {}  name : {}'.format(mapped_key, name))
	          ...
	          if fit_name=='b':
	    	    nuis_p_i+=1
	      	    ...
	      	    hist_fit_b.GetXaxis().SetBinLabel(nuis_p_i,name)
                    if mapped_key: 
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_b'].SetBinContent(mapped_counters[mapped_key],nuis_x.getVal())
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_b'].SetBinError(mapped_counters[mapped_key],nuis_x.getError())
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_b'].GetXaxis().SetBinLabel(mapped_counters[mapped_key],name)
	          if fit_name=='s':
	      	    ...
	      	    hist_fit_s.GetXaxis().SetBinLabel(nuis_p_i,name)
                    if mapped_key: 
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_s'].SetBinContent(mapped_counters[mapped_key],nuis_x.getVal())
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_s'].SetBinError(mapped_counters[mapped_key],nuis_x.getError())
                       mapped_hists[mapped_key][mapped_key+'_prefit_fit_s'].GetXaxis().SetBinLabel(mapped_counters[mapped_key],name)
...
#                  if nuis_x.getError()/sigma_p >=1:
#                  print('fit_name : {}  name : {}  prefit mean : {}  prefit sigma : {}  postfit val : {}  postfit err : {}'.format(fit_name, name, mean_p, sigma_p, nuis_x.getVal(), nuis_x.getError()))
                  if mapped_key: 
                     mapped_hists[mapped_key][mapped_key+'_prefit_nuisances'].SetBinContent(mapped_counters[mapped_key],mean_p)
                     mapped_hists[mapped_key][mapped_key+'_prefit_nuisances'].SetBinError(mapped_counters[mapped_key],sigma_p)
                     mapped_hists[mapped_key][mapped_key+'_prefit_nuisances'].GetXaxis().SetBinLabel(mapped_counters[mapped_key],name)
			...
			valShift = (nuis_x.getVal() - mean_p)
                ...
                if fit_name == 'b':
                    ...
                    mapped_hists[mapped_key][mapped_key+'_pull'].SetBinContent(mapped_counters[mapped_key], valShift)
                    mapped_hists[mapped_key][mapped_key+'_pull'].SetBinError(mapped_counters[mapped_key], 0)
                    mapped_hists[mapped_key][mapped_key+'_pull'].GetXaxis().SetBinLabel(mapped_counters[mapped_key], name)
                if options.abs:
...
if options.plotfile:
    ...
    for key, val in mapped_hists.items():
       canvas_nuis = ROOT.TCanvas(key+'_nuisance', key+'_nuiscane', 1800, 900)
       canvas_nuis.SetBottomMargin(0.3)

       hist_fit_s_tmp = val[key+'_prefit_fit_s']
       hist_fit_b_tmp = val[key+'_prefit_fit_b']
       hist_prefit_tmp = val[key+'_prefit_nuisances']
       hist_pull_tmp = val[key+'_pull']

       hist_fit_e_s = hist_fit_s_tmp.Clone(key+'_errors_s')
       hist_fit_e_b = hist_fit_b_tmp.Clone(key+'_errors_b')

       gr_fit_s = getGraph(hist_fit_s_tmp,-0.1)
       gr_fit_b = getGraph(hist_fit_b_tmp, 0.1)
       gr_fit_s.SetLineColor(ROOT.kRed)
       gr_fit_s.SetMarkerColor(ROOT.kRed)
       gr_fit_b.SetLineColor(ROOT.kBlue)
       gr_fit_b.SetMarkerColor(ROOT.kBlue)
       ...
       gr_fit_b.SetLineWidth(2)
       gr_fit_s.SetLineWidth(2)
       hist_prefit_tmp.SetLineWidth(2)
       hist_prefit_tmp.SetTitle("Nuisance Paramaeters")
       hist_prefit_tmp.SetLineColor(ROOT.kBlack)
       hist_prefit_tmp.SetFillColor(ROOT.kGray)
       hist_prefit_tmp.SetMaximum(3)
       hist_prefit_tmp.SetMinimum(-3)
       hist_prefit_tmp.SetStats(0)
       hist_prefit_tmp.Draw("E2")
       hist_prefit_tmp.Draw("histsame")
       gr_fit_b.Draw("EPsame")
       gr_fit_s.Draw("EPsame")
       canvas_nuis.SetGridx()
       canvas_nuis.RedrawAxis()
       ...
       canvas_nuis.SaveAs(key+'_nuisance.pdf')
       fout.WriteTObject(canvas_nuis)

##########
       canvas_pferrs = ROOT.TCanvas(key+"_post_fit_errs", key+"_post_fit_errs", 1800, 900)
       canvas_pferrs.SetBottomMargin(0.3)

       for b in range(1,hist_fit_e_s.GetNbinsX()+1):
          hist_fit_e_s.SetBinContent(b,hist_fit_s_tmp.GetBinError(b)/hist_prefit_tmp.GetBinError(b))
          hist_fit_e_b.SetBinContent(b,hist_fit_b_tmp.GetBinError(b)/hist_prefit_tmp.GetBinError(b))
          hist_fit_e_s.SetBinError(b,0)
          hist_fit_e_b.SetBinError(b,0)
       hist_fit_e_s.SetFillColor(ROOT.kRed)
       hist_fit_e_b.SetFillColor(ROOT.kBlue)
       ...
       canvas_pferrs.SaveAs(key+'_post_fit_errs.pdf')
       fout.WriteTObject(canvas_pferrs)

#######
       canvas_pull = ROOT.TCanvas(key+"_pull", key+"_pull", 1800, 900)
       canvas_pull.SetBottomMargin(0.3)

       hist_pull_tmp.SetMaximum(3.0)
       hist_pull_tmp.SetMinimum(-3.0)
       hist_pull_tmp.SetStats(0)
       hist_pull_tmp.Draw("hist")
       leg_rat=ROOT.TLegend(0.7,0.8,0.89,0.89)
       leg_rat.SetFillColor(0)
       leg_rat.SetTextFont(42)
       leg_rat.AddEntry(hist_pull_tmp, '(postfit - prefit)/#sigma_{prefit}')
       leg_rat.Draw() 

       canvas_pull.RedrawAxis()

       canvas_pull.SaveAs(key+'_pull.pdf')
       fout.WriteTObject(canvas_pull)
