...
for i in range(fpf_s.getSize()):
    ...
    if not (("signal_stat_unc" in name) or ("rare_stat_unc" in name) or ("ttz_stat_unc" in name) or ("zinv_stat_unc" in name)) : real_binCnt += 1
    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
        if nuis_x == None:
            ...
            if nuis_p != None:
	        if options.plotfile: 
                  if not (("signal_stat_unc" in name) or ("rare_stat_unc" in name) or ("ttz_stat_unc" in name) or ("zinv_stat_unc" in name)):
	            if fit_name=='b':
	    	      nuis_p_i+=1
...
#    hist_prefit.GetXaxis().SetRangeUser(0, real_binCnt-1)
#    hist_prefit.GetXaxis().SetRangeUser(0, 100)
#    hist_prefit.GetXaxis().SetRangeUser(100, 200)
    hist_prefit.GetXaxis().SetRangeUser(200, real_binCnt-1)
    ...
    for b in range(1,hist_fit_e_s.GetNbinsX()+1): 
      if hist_prefit.GetBinError(b) == 0: continue
    ...
    hist_fit_e_b.GetXaxis().SetRangeUser(0, real_binCnt-1)
